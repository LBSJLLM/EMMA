...
class MediumTermMemory:
	...
	def __post_init__(self) -> None:
		...
		self.child_clip_ids = [cid if isinstance(cid, UUID) else UUID(str(cid)) for cid in self.child_clip_ids]
		self.embedding = _ensure_float_list(self.embedding)
	...
	def from_dict(d: Dict[str, Any]) -> MediumTermMemory:
		return MediumTermMemory(
			task_id=UUID(d["task_id"]) if not isinstance(d.get("task_id"), UUID) else d["task_id"],
			topic=d["topic"],
			...
			child_clip_ids=[UUID(cid) if not isinstance(cid, UUID) else cid for cid in d["child_clip_ids"]],
			embedding=_ensure_float_list(list(d["embedding"])),
		)
